...
class CycleCenterNetHead(BaseDenseHead, BBoxTestMixin):
        ...
        for img_id in range(len(img_metas)):
            result_list.append(
                self._get_bboxes_single(
                    ...
                    center2vertex_preds[0][img_id : img_id + 1, ...],
    ...
    def _get_bboxes_single(
        ...
        print(
            f"before in _get_bboxes_single batch_det_bboxes = {batch_det_bboxes.shape}, batch_labels = {batch_labels.shape}"
        )
    ...
    def decode_heatmap(
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(center_heatmap_pred, k=k)
        ...
        tl = transpose_and_gather_feat(c2v_pred[:, 0:2, ...], batch_index)
        tr = transpose_and_gather_feat(c2v_pred[:, 2:4, ...], batch_index)
        br = transpose_and_gather_feat(c2v_pred[:, 4:6, ...], batch_index)
        bl = transpose_and_gather_feat(c2v_pred[:, 6:8, ...], batch_index)
        offset = transpose_and_gather_feat(offset_pred, batch_index)
        topk_xs = topk_xs + offset[..., 0]
        topk_ys = topk_ys + offset[..., 1]
        tl_x = (topk_xs + tl[..., 0]) * (inp_w / width)
        tl_y = (topk_ys + tl[..., 1]) * (inp_h / height)
        tr_x = (topk_xs + tr[..., 0]) * (inp_w / width)
        tr_y = (topk_ys + tr[..., 1]) * (inp_h / height)
        br_x = (topk_xs + br[..., 0]) * (inp_w / width)
        br_y = (topk_ys + br[..., 1]) * (inp_h / height)
        bl_x = (topk_xs + bl[..., 0]) * (inp_w / width)
        bl_y = (topk_ys + bl[..., 1]) * (inp_h / height)

        batch_bboxes = torch.stack([tl_x, tl_y, tr_x, tr_y, br_x, br_y, bl_x, bl_y], dim=2)
        batch_bboxes = torch.cat((batch_bboxes, batch_scores[..., None]), dim=-1)
        print(batch_bboxes.shape, batch_topk_labels.shape)
